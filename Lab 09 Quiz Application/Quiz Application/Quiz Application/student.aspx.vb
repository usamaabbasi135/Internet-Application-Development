...
Public Class student
    ...
    Private Sub update_UserProfile(label_UserProfile As Label)
        ...
        Try
            con.Open()
            adapter.Fill(dsQuizDB, "Users_Login_Credentials")

            'specific user data
            cmd.CommandText = "SELECT StudentID, UserID, Marks, Semester, Department FROM Student_Table Where userID = " & UserID
            adapter.Fill(dsQuizDB, "Student_Table")

            Dim numberRows As Integer = dsQuizDB.Tables.Count
            label_UserProfile.Text = numberRows
        ...
        Try
            ...
            Dim UserLogin_StudentTable As New DataRelation("UserLogin_StudentTable",
            dsQuizDB.Tables("Users_Login_Credentials").Columns("UserID"),
            dsQuizDB.Tables("Student_Table").Columns("UserID"))

            dsQuizDB.Relations.Add(UserLogin_StudentTable)

            Dim table As DataTable = dsQuizDB.Tables("Student_Table")
            ...
            studentRow = dsQuizDB.Tables("Student_Table").Rows.Item(0)
            Console.WriteLine(studentRow("UserID"))
            label_UserProfile.Text &= studentRow("UserID")
            userRow = studentRow.GetParentRow(UserLogin_StudentTable)
